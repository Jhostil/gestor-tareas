-- 1. CAMBIAR LA SESIÓN A LA BASE DE DATOS CONECTABLE REAL
ALTER SESSION SET CONTAINER = XEPDB1;

-- 2. CAMBIAR EL ESQUEMA AL USUARIO DE NUESTRA APLICACIÓN (El que definiste en las variables de Docker)
...
CREATE OR REPLACE PACKAGE TASK_PKG AS
    ...
    PROCEDURE CREATE_TASK(p_title IN VARCHAR2, p_description IN VARCHAR2, p_completed IN NUMBER, p_id OUT NUMBER);
...
CREATE OR REPLACE PACKAGE BODY TASK_PKG AS
    ...
    PROCEDURE CREATE_TASK(p_title IN VARCHAR2, p_description IN VARCHAR2, p_completed IN NUMBER, p_id OUT NUMBER) IS
    BEGIN
        INSERT INTO TASKS (TITLE, DESCRIPTION, COMPLETED) VALUES (p_title, p_description, p_completed) RETURNING TASK_ID INTO p_id;
    END;
